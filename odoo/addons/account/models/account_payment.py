...
class AccountPayment(models.Model):
    ...
    destination_account_other = fields.Many2one('account.account', 'Compte cible', domain="[('company_id', '=', company_id)]")
    use_taxe = fields.Boolean('Taxes compris', default=False)
    ...
    def _prepare_payment_display_name(self):
        ...
        return {
            ...
            'inbound-other': _("Autre règlement"),
            'outbound-other': _("AUtre paiement"),
        }
    ...
    def _prepare_move_line_default_vals(self, write_off_line_vals=None):
        ...
        montant = 0.0
        montant_taxe = (self.amount * self.tax_ids[0].amount)/100 if self.tax_ids else 0.0
        if self.payment_type == 'inbound':
            if self.tax_ids:
                if self.tax_ids[0].price_include is True:
                    montant = self.amount - montant_taxe
                else:
                    montant = self.amount + montant_taxe
            else:
                montant = self.amount
            ...
            # Receive money.
            liquidity_amount_currency = montant #self.amount
        ...
        # if self.tax_ids:
        #     if self.tax_ids[0].price_include is True:
        #         montant_ht = self.amount - montant_taxe
        #         counterpart_balance = -montant_ht - write_off_balance
        #     else:
        #         montant_ttc = self.amount + montant_taxe
        #         counterpart_balance = -montant_ttc - write_off_balance
        # else:
        ...
        #=== Si taxe renseigner====
        if self.tax_ids:
            default_tax_account = self.env['account.tax.repartition.line'].search(['&',('invoice_tax_id', '=', self.tax_ids[0].id), ('repartition_type', '=', 'tax')], limit=1).account_id
            counter_part_taxe = -montant_taxe - write_off_balance
            montant_incule = self.amount - montant_taxe
            if self.tax_ids[0].price_include is True:
                line_vals_list.append({
                    ...
                    'amount_currency': counterpart_amount_currency,
                    'currency_id': currency_id,
                    'debit': montant_incule if counterpart_balance > 0.0 else 0.0,
                    'credit': -montant_incule if counterpart_balance < 0.0 else 0.0,
                    'partner_id': self.partner_id.id or False,
                    'account_id': self.destination_account_id.id,
                    #===Ajout taxe===
                    'tax_ids': self.tax_ids,
                ...
                line_vals_list.append({
                    ...
                    'amount_currency': counterpart_amount_currency,
                    'currency_id': currency_id,
                    'debit': counter_part_taxe if counterpart_balance > 0.0 else 0.0,
                    'credit': -counter_part_taxe if counterpart_balance < 0.0 else 0.0,
                    'partner_id': self.partner_id.id or False,
                    'account_id': default_tax_account.id,
                })
            else:
                line_vals_list.append({
                    'name': self.payment_reference or default_line_name,
                    ...
                    'amount_currency': counterpart_amount_currency,
                    'currency_id': currency_id,
                    'debit': self.amount if counterpart_balance > 0.0 else 0.0,
                    'credit': self.amount if counterpart_balance < 0.0 else 0.0,
                    'partner_id': self.partner_id.id or False,
                    'account_id': self.destination_account_id.id,
                    #===Ajout taxe===
                    'tax_ids': self.tax_ids,
    ...
    def _compute_is_internal_transfer(self):
        for payment in self:
            ...
            # #==MODIF HARNETPROD==
            # is_account_ok = payment.destination_account_id and payment.destination_account_id == self.env[
    ...
    @api.depends('journal_id', 'partner_type', 'is_internal_transfer')
    def _compute_destination_account_id(self):
        ...
        for pay in self:
            ...
            elif pay.partner_type == 'other':
                if pay.partner_id:
                    pay.destination_account_id = pay.destination_account_other
                else:
                    pay.destination_account_id = pay.destination_account_other
    ...
    # HACK :
    #   1. Store value of destination_account_id,on change event, in a global variable which name is "destination_account_id_shadow".
    #   2. Update destination_account_id field directly from this saved global variable by using this custom method : update_destination_account_id_in_db()
    #   3. Call the above method inside "create" and "write" methods in order to update value in database during creation or edit action

    # @api.onchange('destination_account_id')
    # def _onchange_destination_account_id(self):
    #     if self.destination_account_id.id:
    #         self.env.registry['destination_account_id_shadow'] = str(self.destination_account_id.id)
    #
    # @api.model
    # def update_destination_account_id_in_db(self):
    #     cr = self.env.cr
    #     query = "UPDATE account_payment SET destination_account_id = %s WHERE destination_account_id IS NULL" % (self.env.registry.get('destination_account_id_shadow'))
    #     cr.execute(query)
    #     cr.commit()
    #     pass
    ...
    def write(self, vals):
        ...
        # self.update_destination_account_id_in_db()
        return res
