...
class AccountAnalyticDefault(models.Model):
    ...
    product_categ_id = fields.Many2one('product.category', 'Product Category', help="Select category for the current product")
    ...
    department_id = fields.Many2one('hr.department', ondelete="restrict", string='Department', domain=[('patient_department', '=', True)])
    ...
    @api.model
    def account_get(self, product_id=None, partner_id=None, account_id=None, user_id=None, date=None, company_id=None, product_categ_id=None, department_id=None):
        domain = []
        if product_id:
            domain += ['|', ('product_id', '=', product_id)]
        domain += [('product_id', '=', False)]
        if product_categ_id:
            domain += ['|', ('product_categ_id', '=', product_categ_id)]
        ...
        for rec in self.search(domain):
            ...
            if rec.product_categ_id: index += 1
