...
class AccountMove(models.Model):
    ...
    description_fact = fields.Char(string='Description', tracking=True)
    ...
    # ====== Reference paiement & split associé =======
    mode_payment = fields.Many2one('account.journal', string="Mode de paiement",
                                   domain="[('company_id', '=', company_id), ('type', 'in', ('bank', 'cash'))]",
                                   tracking=True, states={'draft': [('readonly', False)]})
    ref_payment = fields.Char(string="Numéro de paiement", tracking=True, states={'draft': [('readonly', False)]})
    fact_associe = fields.Many2one('account.move', copy=False, string="Facture associée", tracking=True,
                                   states={'draft': [('readonly', False)]})
    ...
    def export_to_excel(self):
        ...
        #== REMPLLIR LA COLONNE FOURNISSEUR ET CLIENT ==
        params = self.env.context
        active_model = params.get('active_model')
        move_type = params.get('default_move_type')
        fcd_value = ''
        if active_model == "account.move" and move_type == "out_invoice":
            fcd_value = "C"
        elif active_model == "account.move" and move_type == "in_invoice":
            fcd_value = "F"


        # Define the columns and their headers
        columns = [
            ('fcd', 'Fournisseur (F) Client (C) Débours (D)'),
            ('le', 'Local (L) ou Etranger ( E)'),
            ('partner_id.mg_nif_code', 'NIF (Online) (10 chiffres)'),
            ('partner_id.name', 'Raison sociale'),
            ('partner_id.mg_stat_code', 'STAT'),
            ('partner_id.street', 'Adresse'),
            ('amount_untaxed_signed', 'Montant HT (Ar)'),
            ('amount_tax_signed', 'TVA (Ar)'),
            ('name', 'Facture'),
            ("date_facturation", 'Date Facture (jj/mm/aaaa)'),
            ('type_service', 'Bien (B) ou Service (S) ou Immobilisation (I)'),
            ('operation_label', 'Libellé opération'),
            ("date_paiement", 'Date paiement (jj/mm/aaaa)'),
            ('invoice_month', 'Mois (en chiffre : mois de l\'opération)'),
            ('invoice_year', 'Année'),
            ('obs', 'Observations'),
            ('dau_number', 'N° DAU'),
        ]
        ...
        # Write headers
        for col_num, col_header in enumerate([col[1] for col in columns]):
            worksheet.write(0, col_num, col_header)

        # Compute fields
        def get_combined_value(record, field_names):
            value = record
            try:
                for field_name in field_names:
                    value = getattr(value, field_name)
                    if value in [False, None]:
                        value = 0.0 if field_name in ['amount_untaxed_signed', 'amount_tax_signed'] else ''
                    if isinstance(value, models.BaseModel) and field_name != field_names[-1]:
                        continue
        ...
        row_num = 1
        for record in self:
            for col_num, col_name in enumerate([col[0] for col in columns]):
                if col_name == "fcd":
                    value = fcd_value
                elif col_name == 'operation_label':
                    company_name = record.partner_id.name
                    value = f'FACTURE {company_name}'
                elif col_name == 'date_facturation':
                    date_fact = record.invoice_date.strftime('%d/%m/%Y')
                    value = str(date_fact)
                elif col_name == 'date_paiement':
                    date_paid = record.invoice_date_due.strftime('%d/%m/%Y')
                    value = str(date_paid)
                elif col_name == 'invoice_month':
                    invoice_date = record.invoice_date
                    month_str = babel_format_date(invoice_date, 'MMMM', locale='fr_FR')
                    value = str(month_str).upper()
                elif col_name == 'invoice_year':
                    invoice_date = record.invoice_date
                    value = str(invoice_date.year)
                elif col_name == "amount_tax_signed" and fcd_value == "F":
                    value = get_combined_value(record, ['amount_tax_signed']) * -1 if get_combined_value(record, ['amount_tax_signed']) != 0.0 else get_combined_value(record, ['amount_tax_signed'])
                elif col_name == "amount_untaxed_signed" and fcd_value == "F":
                    value = get_combined_value(record, ['amount_untaxed_signed']) * -1 if get_combined_value(record, ['amount_untaxed_signed']) != 0.0 else get_combined_value(record, ['amount_untaxed_signed'])
                else:
                    field_names = col_name.split('.')
                    value = get_combined_value(record, field_names)
        ...
        attachment = self.env['ir.attachment'].create({
            'name': f'DGI-Annexe-TVA_{formatted_now}.xlsx',
            'type': 'binary',
    ...
    @api.model
    def get_invoice_by_company(self):
            company_id = self.env.user.company_id.id
            return {
                    'name': _('Get invoice by company'),
                    ...
                    'domain': [('move_type', '=', 'out_invoice'), ('patient_id', '!=', False),
...
class AccountMoveLine(models.Model):
    ...
    def _onchange_product_id(self):
        for line in self:
            ...
            line.analytic_account_id = line.product_id.categ_id.property_account_analytic_categ_id
            account_tags_categ_id = []
            for rec in line.product_id.categ_id.property_account_tags_categ_id:
                account_tags_categ_id.append(rec.id)
                
            line.analytic_tag_ids = [(6, 0, account_tags_categ_id)]
