...
#        project_id = request.env['project.project'].sudo().search([('code', '=', post.get('project_code'))], limit=1)
        ...
        issue_template = http.request.env.ref('website_job_workorder_request.custom_wo_email_template_job_order_to_customer')
        #issue_template.sudo().with_context(local_context).send_mail(request.uid, force_send=False)
        issue_template.sudo().send_mail(workorder_id.id, force_send=False)
    ...
    def custom_wo_joborder_print(self, custom_wo_joborder, access_token=None, **kw):
        ...
        pdf, _ = request.env.ref('website_job_workorder_request.custom_wo_joborder_report').sudo()._render_qweb_pdf([custom_wo_joborder])
        pdfhttpheaders = [
