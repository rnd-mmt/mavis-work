...
class StockMove(models.Model):
    ...
    def _action_assign(self):
        ...
        """
        if self.picking_id and self.picking_id.inter_company_transfer_id and "incoming" in \
                self.picking_id.picking_type_id.mapped("code") and self.move_orig_ids:
            if all(all(origin_move.state != "done" for origin_move in move.move_orig_ids) for move in self):
                return False
        ...
        for move in self.filtered(lambda m: m.state not in ["draft", "done", "cancel"]):
            ...
            ict_sale_lot_assign = bool(customer_move.sale_line_id.ict_line_id and not move.move_orig_ids)
            ict_purchase_lot_assign = bool(
                ict and ict.type == "ict" and not ict.reverse_inter_company_transfer_ids and move.purchase_line_id)
            ict_internal_lot_assign = bool(ict and not move.move_orig_ids and ict.type in ["internal", "int_reverse"])
            reverse_sale_ict_lot_assign = bool(ict and ict.type == "ict_reverse" and move.sale_line_id)
            reverse_po_ict_lot_assign = bool(ict and ict.type == "ict_reverse" and
                                             move.location_id == move.picking_type_id.warehouse_id.lot_stock_id)
        ...
        for origin_move in origin_moves:
            ...
            for move_line in move_lines:
                ...
                if move_line.product_id.tracking == 'serial':
                    ...
                    if serial and serial.product_qty == 0.0:
                        vals.update({"lot_id": serial.id})
    ...
    def _prepare_phantom_move_values(self, bom_line, product_qty, quantity_done):
        ...
        if self.picking_id.inter_company_transfer_id:
            ...
            origin_move = origin_picking.move_lines.filtered(lambda x:
