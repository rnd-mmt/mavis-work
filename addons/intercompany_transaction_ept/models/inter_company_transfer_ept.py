...
class InterCompanyTransferEpt(models.Model):
    ...
    # ===materail_purchase_request
    department_id = fields.Many2one('hr.department', string='Département', copy=True,
    ...
    def _compute_log_ids(self):
        """
        Counts Log book records of ICT.
        @author: Maulik Barad.
        """
        for ict in self:
    ...
    def create(self, vals):
        ...
        if vals.get("type", "") in ["ict", ""]:
            ...
            sequence_id = self.env.ref("intercompany_transaction_ept.ir_sequence_reverse_inter_company_transaction").ids
    ...
    def fields_view_get(self, view_id=None, view_type="form", toolbar=False, submenu=False):
        ...
        res = super(InterCompanyTransferEpt, self).fields_view_get(view_id, view_type, toolbar, submenu)
        ...
        for record in self:
            # if record.with_context(context).check_user_validation():
                ...
                if transfer_type in ["internal", "int_reverse"]:
                    internal_transfers = record.sudo().create_internal_transfer_ept()
                ...
                # Processing the Reverse ICT.
                    ...
                    processed = record.with_context(context).sudo().process_reverse_ict()
                    if not processed:
                        continue

                # Processing the ICT.
                else:
                    record.with_company(record.source_company_id).sudo().create_ict_sale_order()
                    record.with_company(record.destination_company_id).sudo().create_ict_purchase_order()

                    record.with_context(context).sudo().process_ict_by_workflow()
    ...
    def get_group_and_route(self, destination_warehouse):
        ...
        route_ids = self.env['stock.location.route'].search([('supplied_wh_id', '=', destination_warehouse.id),
        ...
        if not route_ids:
            ...
            if self._context.get("auto_process"):
                log_book = self.env['inter.company.transfer.log.book.ept'].create_ict_log_book(self, "reverse")
        ...
        for record in self:
            ...
            for line in record.inter_company_transfer_line_ids:
                if dropship_route and dropship_route in line.product_id.route_ids:
                    ...
                    log_line_vals.append((0, 0, {"ict_message": message, "ict_log_type": "mismatch"}))
                    continue
            ...
            if sale_order_lines:
                ...
                sale_order = sale_obj.sudo().create(order_vals)
                sale_order._compute_tax_id()

            if log_line_vals:
                log_book = self.env['inter.company.transfer.log.book.ept'].create_ict_log_book(self, "ict")
                log_book.write({"ict_log_line_ids": [(6, 0, log_book.ict_log_line_ids.ids)] + log_line_vals})
    ...
    def prepare_ict_sale_order_vals(self):
        ...
        new_order = sale_obj.sudo().new({'partner_id': customer_partner_id.id,
                                  'warehouse_id': self.sudo().source_warehouse_id.id,
                                  'pricelist_id': self.pricelist_id.id,
                                  'department_associe': self.department_id.name,
                                  'company_id': self.sudo().source_company_id.id})
    ...
    def prepare_ict_sale_order_line_vals(self, line):
        ...
        new_so_line.update({'product_uom_qty': line.quantity,
                            'price_unit': line.price,
                            "ict_line_id": line.id,
                            'analytic_tag_ids': default_analytic_account.analytic_tag_ids or line.product_id.categ_id.property_account_tags_categ_id})
    ...
    def create_ict_purchase_order(self):
        ...
        for record in self:
            ...
            if purchase_order_lines:
                ...
                purchase_order = purchase_obj.sudo().create(order_vals)
    ...
    def auto_validate_ict_pickings(self):
        ...
        if self.auto_workflow_id.auto_validate_receipt:
            ...
            moves = vendor_picking.move_lines
            ...
            if picking.state != "assigned":
                if picking.move_lines.move_orig_ids:
                    completed = self.validate_ict_pickings(
                        picking.move_lines.move_orig_ids.picking_id.sorted(lambda x: x.id))
                    if not completed:
                        return False
                picking.action_assign()
                if picking.state != "assigned":
    ...
    def get_destination_lots_for_reverse(self, line):
        ...
        @author: Maulik Barad on Date 30-Dec-2020.
        """
        lot_serial_ids = self.env["stock.production.lot"].search([("name", "in", line.lot_serial_ids.mapped("name")),
                                                                  ("product_id", '=', line.product_id.id),
                                                                  ("company_id", "=", self.destination_company_id.id)])
    ...
    def process_reverse_ict(self):
        ...
        if outgoing_picking:
            moves = self.get_destination_moves(outgoing_picking.move_lines)
        ...
        for done_picking in picking.filtered(lambda x: x.state == "done" and x not in returned_pickings):
            ...
            if return_picking_wizard.picking_id:
                ...
                if move_vals:
                    ...
                    if done_picking.move_lines.move_orig_ids:
                        # For skipping the Partial Returned moves.
                        moves_for_return = done_picking.move_lines.move_orig_ids.filtered(
                            lambda x: x.origin_returned_move_id not in done_picking.move_lines)
    ...
    def reverse_invoices(self):
        ...
        for invoice in invoices:
            reverse_move = account_move_reversal_obj.with_context(context, active_ids=invoice.ids).create({})
    ...
    def prepare_return_move_vals(self, ict_line, picking):
        ...
        """
        move_vals = []
        bom_product = self.check_bom_product(ict_line.product_id)
        if bom_product:
            move_vals += self.prepare_return_move_vals_for_bom(ict_line, bom_product, picking)
        else:
            move = picking.move_lines.filtered(lambda x: x.product_id == ict_line.product_id and x.state == 'done')
    ...
    def prepare_return_move_vals_for_bom(self, ict_line, bom, picking):
        """
        ...
        to_uom = bom.product_uom_id

        factor = from_uom._compute_quantity(1, to_uom) / bom.product_qty
        bom, lines = bom.explode(bom_product, factor, picking_type=bom.picking_type_id)

        for line in lines:
            product = line[0].product_id
            move = picking.move_lines.filtered(lambda x: x.product_id == product and x.state == 'done')
            quantity = ict_line.quantity
            if move.product_uom_qty < quantity:
                quantity = move.product_uom_qty
    ...
    def check_bom_product(self, product):
        ...
        if mrp_module:
            bom_point = self.env['mrp.bom'].sudo()._bom_find(product=product, bom_type='phantom')
            if bom_point:
                return bom_point
        return False
    ...
    def open_related_records(self):
        ...
        elif res_model == "inter.company.transfer.log.book.ept":
            name = "Logs"
            record_ids = self.log_ids.ids
