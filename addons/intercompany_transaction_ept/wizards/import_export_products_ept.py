...
class ImportExportProducts(models.TransientModel):
    ...
    def import_product_list(self):
        ...
        if inter_company_transfer_id:
            log_book = ict_log_obj.create_ict_log_book(inter_company_transfer_id, operation_type="import")
    ...
    def import_products_from_csv(self, ict, log_book):
        ...
        for line in reader:
            if not line.get("Default Code") or not line.get("Default Code").strip():
                raise UserError(_("Unable to process..! Please Provide Default Code of Product..."))

            default_code = line.get("Default Code").strip()
            product = self.get_product_from_sku(default_code, log_book)
            if not product:
                continue
            ...
            lot_ids = self.get_lot_ids(line.get("Lot/Serial Number"), product, quantity, log_book,
                                       ict.source_company_id.id)
            if isinstance(lot_ids, bool):
                continue

            vals = self.prepare_ict_line_vals(ict, product, quantity, price, lot_ids, log_book)
            if not vals:
                continue
    ...
    def get_product_from_sku(self, default_code, log_book):
        """
        This method is used to search product from the sku.
        @param default_code: SKU to find the product.
        @param log_book: Record of the log book.
        @author: Maulik Barad on Date 22-Dec-2020.
        """
        product_obj = self.env["product.product"]
        product = product_obj.search([("default_code", "=", default_code)], limit=1)
        if not product:
            msg = """Default code does not match with any Product. Default code is %s.""" % default_code
            log_book.post_log_line(msg, log_type="mismatch")
        return product

    def get_lot_ids(self, lot_numbers, product, quantity, log_book, company_id):
        """
        ...
        """
        lot_ids = []
        lot_stock_obj = self.env["stock.production.lot"]
    ...
    def prepare_ict_line_vals(self, ict, product, quantity, price, lot_ids, log_book):
        """
    ...
    def export_product_list_as_csv(self, ict_lines):
        ...
        # Adds column headers.
        field_names = ["Default Code", "Qty", "Price", "Lot/Serial Number"]
    ...
    def export_product_list_as_xls(self, ict_lines):
        ...
        worksheet.write(0, 3, "Lot/Serial Number")
        ...
        for line in ict_lines:
            ...
            worksheet.write(row, 3, lot_numbers or "")
            row = row + 1
    ...
    def on_barcode_scanned(self, barcode):
        ...
        lot_serial = self.env["stock.production.lot"].search_read([("name", "=", barcode),
                                                                   ("company_id", "=", company_id)],
