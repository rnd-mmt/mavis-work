...
class ZkMachine(models.Model):
    ...
    # modification atao am resaka mot de passe
    mdp = fields.Char(string='Mot de passe')
    ...
    def clear_attendance(self):
        for info in self:
            try:
                ...
                timeout = 30
                try:
                    zk = ZK(machine_ip, port=zk_port, timeout=timeout, password=mypass, force_udp=False, ommit_ping=False)
                except NameError:
    ...
    def download_attendance(self):
        ...
        for info in self:
            ...
            timeout = 30
            try:
                zk = ZK(machine_ip, port=zk_port, timeout=timeout, password=mypass, force_udp=True, ommit_ping=True)
            except NameError:
                raise UserError(_("Pyzk module not Found. Please install it with 'pip3 install pyzk'."))
            conn = self.device_connect(zk)
            _logger.info(machine_ip)
            _logger.info(zk_port)
            if conn:
