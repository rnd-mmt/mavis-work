...
class StockProductionLot(models.Model):
    ...
    def _onchange_expiration_date(self):
        if self.expiration_date:
            self.removal_date = (self.expiration_date + timedelta(days= -(self.product_id.removal_time))).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            self.use_date = (self.expiration_date + timedelta(days= -(self.product_id.use_time))).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
            self.alert_date = (self.expiration_date + timedelta(days= -(self.product_id.alert_time))).strftime(DEFAULT_SERVER_DATETIME_FORMAT)

#    @api.onchange('expiration_date')
#    def _onchange_expiration_date(self):
#        if self.expiration_date:
#            self.removal_date = (self.expiration_date + timedelta(days= -90)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
#            self.use_date = (self.expiration_date + timedelta(days= -90)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
#            self.alert_date = (self.expiration_date + timedelta(days= -90)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
