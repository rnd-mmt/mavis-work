...
class SaleOrderLine(models.Model):
    ...
    def _compute_qty_delivered_method(self):
        ...
        for line in self:
            if not line.is_expense and line.product_id.type in ['product']:
    ...
    def _onchange_product_uom_qty(self):
        ...
        if self.state == 'sale' and self.product_id.type in ['product'] and self.product_uom_qty < product_uom_qty_origin:
    ...
    def _action_launch_stock_rule(self, previous_product_uom_qty=False):
        ...
        for line in self:
            ...
            if line.state != 'sale' or not line.product_id.type in ('product'):
                continue
    ...
    def _update_line_quantity(self, values):
        precision = self.env['decimal.precision'].precision_get('Product Unit of Measure')
        line_products = self.filtered(lambda l: l.product_id.type in ['product'])
