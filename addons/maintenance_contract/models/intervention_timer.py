...
class ProjectTaskTimer(models.Model):
    ...
    task_timer = fields.Boolean(string='Intervention en cours', default=False)
    is_user_working = fields.Boolean('Is Current User Working', compute='_compute_is_user_working',help="Technical field indicating whether the current user is working. ")
    duration = fields.Float('Durée intervention (Down time)', compute='_compute_duration', store=True)
    ...
    def start_task_intervention(self):
        self.task_timer = True

    def stop_task_intervention(self):
        self.task_timer = False


    def is_equipment_stopped(self):
        task_obj = self.env['project.task'].search([('project_id', '=', self.project_id.id)])
        is_stopped = False
        for task in task_obj:
            if not task.task_timer:
                is_stopped = True
        return is_stopped
