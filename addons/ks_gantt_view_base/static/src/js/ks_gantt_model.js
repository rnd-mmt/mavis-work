odoo.define("ks_gantt_view.Model", function (require) {
    ...
    var ksGanttModel = AbstractModel.extend({
        ...
        load: function (params) {
            ...
            this.ks_gantt_link_info = params.ks_task_link_info ? params.ks_task_link_info : false;
            this.ks_defaultGroupBy = params.ks_defaultGroupBy ? [params.ks_defaultGroupBy] : [];
        ...
        _ksComputeData: function () {
            ...
            if (this.ksDataForGantt && this.ksDataForGantt.groupedBy && this.ksDataForGantt.groupedBy.length) {

                // Remove date field group by.
                this.ksDataForGantt.groupedBy = _.filter(this.ksDataForGantt.groupedBy, function (ksGroupBy) {
                    if (ksGroupBy.indexOf(":") < 0)
                        return ksGroupBy;
                });
            ...
            return this.dp.add(Promise.all([ksGroupDefaultData, ksDefaultData])).then(function (results) {
        ...
        ksFetchDefaultFields: function () {
            ...
            ksRequireFiled = ksRequireFiled.concat(this.ksDataForGantt.groupedBy, Object.values(this.ks_gantt_arch));
            ksRequireFiled = _.filter(ksRequireFiled, function (item) { return item != 'ks_project_row'; });
        ...
        */
        updateTask: function (data) {
            ...
            if (data.constraint_date && this.ks_gantt_arch.ks_constraint_task_date) {
                ...
                ks_data_values[this.ks_gantt_arch.ks_constraint_task_date] = this.conversionToUTCTime(moment(constraint_date));
            }
            ks_data_values[this.ks_gantt_arch.ks_task_start_date] = this.fields[this.ks_gantt_arch.ks_task_start_date].type == 'date' ? moment(date_start).format('YYYY-MM-DD') : this.conversionToUTCTime(moment(date_start));
            ks_data_values[this.ks_gantt_arch.ks_task_end_date] = this.fields[this.ks_gantt_arch.ks_task_end_date].type == 'date' ? moment(date_end).format('YYYY-MM-DD') : this.conversionToUTCTime(moment(date_end));
            if (this.ks_gantt_arch.ks_constraint_task_type) {
                ks_data_values[this.ks_gantt_arch.ks_constraint_task_type] = data.constraint_type;
            }
        ...
        ksUpdateProjectGanttTask: function (data) {
            ...
            if (data.constraint_date && this.ks_gantt_arch.ks_constraint_task_date) {
                var constraint_date = gantt_date_obj(data.constraint_date);
                ks_data_values['ks_constraint_task_date'] = this.conversionToUTCTime(moment(constraint_date));
            }

            ks_data_values['ks_start_datetime'] = this.conversionToUTCTime(moment(date_start));
            ks_data_values['ks_end_datetime'] = this.conversionToUTCTime(moment(date_end));
        ...
        */
        ksCreateLink: function (data) {
            ...
            if (this.modelName == 'project.project') {
                create_dict['ks_source_task_id'] = parseInt(data.source.split('task_')[1]);
                create_dict['ks_target_task_id'] = parseInt(data.target.split('task_')[1]);
            }
            else {
                create_dict[this.ks_gantt_link_info.ks_link_source] = parseInt(data.source);
                create_dict[this.ks_gantt_link_info.ks_link_target] = parseInt(data.target);
            }
            return this._rpc({
                model: 'ks.task.link',
                method: 'create',
