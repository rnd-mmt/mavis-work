odoo.define("ks_gantt_view.ks_recommend_colorpicker", function (require) {
    ...
    var ks_gantt_color_picker_widget = ColorPickerWidget.ColorpickerWidget.include({
        events: _.extend({}, ColorPickerWidget.ColorpickerWidget.prototype.events, {
            'click div.ks_recommend_color': '_KsClickRecommendColor',
        }),

        _KsClickRecommendColor: function (ev) {
            $('.o_colorpicker_widget input.o_hex_input').val($(ev.currentTarget).attr("data-hex")).trigger('change');
        },
    });
