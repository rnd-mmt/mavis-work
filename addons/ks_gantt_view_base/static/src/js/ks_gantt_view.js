odoo.define("ks_gantt_view.View", function (require) {
    ...
    var KsGanttView = AbstractView.extend({
        ...
        init: function (viewInfo, params) {
            ...
            var ks_formViewId = arch.attrs.form_view_id ? parseInt(arch.attrs.form_view_id, 10) : false;
            ...
            var ks_no_drag = (arch.attrs && arch.attrs.ks_no_drag == 'true') ? true : false;
            var ks_hide_links = (arch.attrs && arch.attrs.ks_hide_links == 'true') ? true : false;
            ...
            this.rendererParams.ks_isCreate = this.controllerParams.activeActions.create;
        ...
        ks_manage_gantt_arch: function () {
            ...
            ks_gantt_no_field.forEach(function (field_info) {
                if (this.arch.attrs[field_info]) {
                    try {
                        this.loadParams[field_info] = JSON.parse(this.arch.attrs[field_info]);
                        this.rendererParams[field_info] = JSON.parse(this.arch.attrs[field_info]);

                        if(field_info == 'ks_context'){
                            let ks_context_info = JSON.parse(this.arch.attrs[field_info]);
                            let ks_new_context = Object.assign({}, this.controllerParams.context, ks_context_info);
                            this.controllerParams.context = ks_new_context;
                        }
                        if(field_info == 'ks_export_field'){
                            this.rendererParams[field_info] = JSON.parse(this.arch.attrs[field_info]);
                            this.loadParams[field_info] = JSON.parse(this.arch.attrs[field_info]);
                            this.controllerParams[field_info] = JSON.parse(this.arch.attrs[field_info]);
                        }
                    }
                    catch (err) {
                        console.error(err);
        ...
        */
        ks_get_gantt_no_field_attribute: function () {
            return ['ks_task_link_info', 'ks_gantt_config', 'ks_context', 'ks_export_field']
        },
    });
