odoo.define("ks_gantt_view.Renderer", function (require) {
    ...
    var ksGanttRenderer = AbstractRenderer.extend({
        ...
        ks_renderGantt: function () {
            ...
            if (this.ks_enable_task_dynamic_text === undefined || this.ks_enable_task_dynamic_text) {
            ...
            var ks_extra_space = $('.o_main_navbar').height() + $('.o_control_panel').height() + $('.ks_gantt_right_control').height();
            this.$('.ks_gantt_view_content').height(window.outerHeight - ks_extra_space);
            ...
            gantt.config.ks_attached_events.push(gantt.attachEvent("onTaskLoading", function (task) {
                if (task.deadline)
            ...
            if (this.ks_enable_task_dynamic_progress == undefined || this.ks_enable_task_dynamic_progress) {
...
//            }
            ...
            gantt.config.ks_attached_events.push(gantt.attachEvent("onBeforeTaskDrag", function (id, mode, e) {
                ...
                if (!this.ks_isEdit || gantt.getTask(id).ks_schedule_mode == "auto") {
                    return false;
                }
                return true;
            }.bind(self)));
        ...
        ks_parse_gantt_data: function () {
            ...
            if (!this.state.groupedBy.length) {
                this.state.records.forEach(function (each_task) {
                    this.ks_data_update(each_task, ks_links, ks_data, ks_gantt_fields, false);
                    if (each_task.ks_project_task_json) {
                        var ks_project_task_data = JSON.parse(each_task.ks_project_task_json);
                        ks_project_task_data.forEach(function (each_project_task) {
                            this.ks_project_task_data_update(each_project_task, ks_links, ks_data, each_task.id);
                        }.bind(this));
                    }
                }.bind(this));
            }
                ...
                for (var group_index in this.state.ks_group_by_records) {
                    ...
                    for (var group_lvl in this.state.groupedBy) {
                        ...
                        if (parseInt(group_lvl) < 1) {
                            ...
                            if (each_group[sub_group_name])
                                sub_group = Array.isArray(each_group[sub_group_name]) ? each_group[sub_group_name] : each_group[sub_group_name].toString();
                            ...
                            if (sub_group){
                                ks_data.push({
                                    'id': "group_" + (typeof(sub_group) == 'string' ? sub_group : sub_group[0]),
                                ...
                                parent_group_id = "group_" + (typeof(sub_group) == 'string' ? sub_group : sub_group[0]);
                                ks_id_list.push("group_" + (typeof(sub_group) == 'string' ? sub_group : sub_group[0]));
                            }
                        }
                        // Sub-group needs to be changed its ids to stop duplicate id.
                        if (parseInt(group_lvl) > 0) {
                            if (each_group[sub_group_name])
                                sub_group = Array.isArray(each_group[sub_group_name]) ? each_group[sub_group_name][1] : each_group[sub_group_name].toString();
                                // Check if field type is selection
                                if(this.ks_fieldDetail[sub_group_name].type == 'selection'){
                                    sub_group = this.ks_get_stage_data(sub_group_name, sub_group);
                                }
                            if (sub_group){
                                var new_id = parent_group_id + "_" + sub_group[0];
                                ks_data.push({
                                    'id': new_id,
                                    'text': sub_group,
                                    'parent': parent_group_id,
                        ...
                        if (sub_group)
                            ks_group_by_dict[sub_group_name] = sub_group;
                    }
...
//                            }
                        ...
                        if (this.ks_compare_group_by_data(ks_each_task[sub_group_name], each_group[sub_group_name])) {
        ...
        */
        ks_compare_group_by_data: function(ks_applied_grouped, ks_current_record){
            if (Array.isArray(ks_applied_grouped) && Array.isArray(ks_applied_grouped) &&
                (ks_applied_grouped.length == ks_current_record.length)) {
                    for (var ks_index=0; ks_index<ks_current_record.length; ks_index++){
                ...
                return ks_applied_grouped == ks_current_record;
            }
        },
        ...
        willStart: function () {
            ...
            var ksGetTheme = this._rpc({
                ...
                if (!result.ks_gantt_view_theme) result.ks_gantt_view_theme = "dhtmlxgantt_terrace.css";
                var ks_gantt_lib_theme = '/ks_gantt_view_base/static/lib/gantt_7.0.11_commercial/codebase/skins/' + result.ks_gantt_view_theme;
                ajax.loadCSS(ks_gantt_lib_theme);

                // Apply custom theme css files.
                if (result.ks_gantt_view_theme == 'dhtmlxgantt_meadow.css') {
                    ajax.loadCSS('/ks_gantt_view_base/static/src/css/skins/ks_meadow.css');
                }
                if (result.ks_gantt_view_theme == 'dhtmlxgantt_broadway.css') {
                    ajax.loadCSS('/ks_gantt_view_base/static/src/css/skins/ks_broadway.css');
                }
                if (result.ks_gantt_view_theme == 'dhtmlxgantt_material.css') {
                    ajax.loadCSS('/ks_gantt_view_base/static/src/css/skins/ks_material.css');
                }
                if (result.ks_gantt_view_theme == 'dhtmlxgantt_contrast_white.css') {
                    ajax.loadCSS('/ks_gantt_view_base/static/src/css/skins/ks_contrast-white.css');
                }
                if (result.ks_gantt_view_theme == 'dhtmlxgantt_contrast_black.css') {
                    ajax.loadCSS('/ks_gantt_view_base/static/src/css/skins/ks_contrast-black.css');
                }
                if (result.ks_gantt_view_theme == 'dhtmlxgantt_terrace.css') {
                    ajax.loadCSS('/ks_gantt_view_base/static/src/css/skins/ks_terrace.css');
                }
...
//                    this.ks_resource_panel();
         ...
         *
         */
        ks_data_update: function (each_task, ks_links, ks_data, ks_gantt_fields, parent_group_id) {
            if (each_task[ks_gantt_fields.ks_task_link]) {
                JSON.parse(each_task[ks_gantt_fields.ks_task_link]).forEach((item, index) => {
                    ks_links.push({
                        'id': item.id,
                        'source': item.source,
                        'target': item.target,
            ...
            if (each_task[ks_gantt_fields.ks_resource_hours_available])
                ks_resource_hours_available = JSON.parse(each_task[ks_gantt_fields.ks_resource_hours_available])
            ...
            var ks_gantt_data = {
                ...
                'color': each_task[ks_gantt_fields.ks_task_color] ? each_task[ks_gantt_fields.ks_task_color] : '#7C7BAD',
                'start_date': this.ks_fieldDetail[ks_gantt_fields.ks_task_start_date].type == 'date' ? moment(each_task[ks_gantt_fields.ks_task_start_date]).toDate() : this.ksConversionUtcToLocal(moment(each_task[ks_gantt_fields.ks_task_start_date])),
                'end_date': this.ks_fieldDetail[ks_gantt_fields.ks_task_end_date].type == 'date' ? moment(each_task[ks_gantt_fields.ks_task_end_date]).toDate() : this.ksConversionUtcToLocal(moment(each_task[ks_gantt_fields.ks_task_end_date])),
                'mark_as_important': each_task[ks_gantt_fields.ks_mark_as_important],
                'deadline': gantt.config.ks_toggle_deadline ? each_task[ks_gantt_fields.ks_task_deadline] : false,
                'ks_deadline_tooltip': each_task[ks_gantt_fields.ks_task_deadline] ? moment(each_task[ks_gantt_fields.ks_task_deadline]).toDate() : false,
                ...
                'parent': each_task[ks_gantt_fields.ks_parent_task] ? each_task[ks_gantt_fields.ks_parent_task][0] : parent_group_id,
                ...
                'auto_scheduling': each_task[ks_gantt_fields.ks_schedule_mode] == 'auto' ? true : false,
                'constraint_type': each_task[ks_gantt_fields.ks_constraint_task_type] ? each_task[ks_gantt_fields.ks_constraint_task_type] : false,
                'ks_constraint_type': each_task[ks_gantt_fields.ks_constraint_task_type] ? each_task[ks_gantt_fields.ks_constraint_task_type] : false,
                'constraint_date': each_task[ks_gantt_fields.ks_constraint_task_date] ? this.ksConversionUtcToLocal(moment(each_task[ks_gantt_fields.ks_constraint_task_date])) : false,
                'constraint_date_enable': each_task[ks_gantt_fields.ks_constraint_task_date] ? true : false,
                'stage_id': this.ks_get_stage_data(ks_gantt_fields.ks_task_stage_id, each_task[ks_gantt_fields.ks_task_stage_id]),
                'unscheduled': each_task[ks_gantt_fields.ks_unschedule],
                'ks_owner_task': each_task[ks_gantt_fields.ks_owner_task],
                'user': each_task[ks_gantt_fields.ks_owner_task] ? each_task[ks_gantt_fields.ks_owner_task][0] : 0,
                'resource_working_hours': each_task[ks_gantt_fields.ks_resource_hours_per_day],
                ...
                'ks_enable_task_duration': each_task[ks_gantt_fields.ks_enable_task_duration],
            ...
            if (this.ks_export_field){
                for(var index=0; index<=this.ks_export_field.length; index++){
                   ks_gantt_data[this.ks_export_field[index]] = each_task[this.ks_export_field[index]];
                }
            }

            if (ks_gantt_data.start_date instanceof Date && isFinite(ks_gantt_data.start_date) &&
                ks_gantt_data.end_date instanceof Date && isFinite(ks_gantt_data.end_date)){
                    ks_data.push(ks_gantt_data);
            }
            else{
                gantt.message({
                    type: "warning",
                    text: _t(ks_gantt_data.text + " not shown due to start/end date is not found.")
                });
            }

            if (!gantt.config.ks_owner_task_list) {
                gantt.config.ks_owner_task_list = [{ key: "0", label: "N/A", id: "0", text: "N/A" }];
            ...
            if (each_task[ks_gantt_fields.ks_owner_task] && each_task[ks_gantt_fields.ks_owner_task][0] && gantt.config.ks_owner_task_dict.indexOf(each_task[ks_gantt_fields.ks_owner_task][0]) < 0) {
                gantt.config.ks_owner_task_dict.push(each_task[ks_gantt_fields.ks_owner_task][0]);
        ...
        ks_get_stage_data: function(stage_field, stage_data){
            if (stage_field && this.ks_fieldDetail[stage_field].type == 'selection'){
                ...
                this.ks_fieldDetail[stage_field].selection.forEach(function (selection_data) {
        ...
        ks_project_task_data_update: function (each_project_task, ks_links, ks_data, project_id) {

            var ks_resource_hours_available = {};
            if (each_project_task.ks_resource_hours_available)
                ks_resource_hours_available = JSON.parse(each_project_task.ks_resource_hours_available)

            if (each_project_task.ks_task_link_json) {
                JSON.parse(each_project_task.ks_task_link_json).forEach((item, index) => {
                    ks_links.push({
                        'id': item.id,
            ...
            var ks_gantt_datas = {
                ...
                'color': each_project_task.ks_task_color ? each_project_task.ks_task_color : '#7C7BAD',
                'start_date': this.ksConversionUtcToLocal(moment(each_project_task.ks_task_start_date)),
                'end_date': this.ksConversionUtcToLocal(moment(each_project_task.ks_task_end_date)),
                'parent': each_project_task.parent_id ? each_project_task.parent_id : project_id,
                'mark_as_important': each_project_task.mark_as_important,
                'deadline': gantt.config.ks_toggle_deadline ? each_project_task.deadline : false,
                'ks_deadline_tooltip': each_project_task.deadline ? moment(each_project_task.deadline).toDate() : false,
...
//                'ks_allow_parent_task': each_project_task.ks_allow_subtask,
                ...
                'auto_scheduling': each_project_task.ks_schedule_mode == 'auto' ? true : false,
                'constraint_type': each_project_task.constraint_type ? each_project_task.constraint_type : false,
                'constraint_date': each_project_task.constraint_date ? this.ksConversionUtcToLocal(moment(each_project_task.constraint_date)) : false,
                'constraint_date_enable': each_project_task.constraint_date ? true : false,
                'stage_id': each_project_task.stage_id ? each_project_task.stage_id : undefined,
                ...
                'user': each_project_task.ks_owner_task ? each_project_task.ks_owner_task[0] : 0,
                'type': each_project_task.type,
                'ks_resource_hours_available': ks_resource_hours_available,
                'project_id': each_project_task.project_id,
                'create_date': this.ksConversionUtcToLocal(moment(each_project_task.create_date)),
                'project_id': each_project_task.project_id,
            ...
            if (this.ks_export_field){
                for(var index=0; index<=this.ks_export_field.length; index++){
                   ks_gantt_datas[this.ks_export_field[index]] = each_task[this.ks_export_field[index]];
                }
            }

            if (ks_gantt_datas.start_date instanceof Date && isFinite(ks_gantt_datas.start_date) &&
                ks_gantt_datas.end_date instanceof Date && isFinite(ks_gantt_datas.end_date)){
            ...
            if (each_project_task.ks_owner_task && each_project_task.ks_owner_task[0] && gantt.config.ks_owner_task_dict.indexOf(each_project_task.ks_owner_task[0]) < 0) {
                gantt.config.ks_owner_task_dict.push(each_project_task.ks_owner_task[0]);
                gantt.config.ks_owner_task_list.push({
        ...
        ks_linked_task_info: function () {
            var highlightTasks = [], highlightSearch = {};
            ...
            gantt.templates.task_class = function (start, end, task) {
                if (highlightSearch[task.id])
                    return "ks_highlighted_task";
                return "";
            };

            gantt.config.ks_attached_events.push(gantt.attachEvent("onEmptyClick", function () {
                reset();
                return true;
            }));
        },
        ...
        ks_task_dynamic_content: function () {
            ...
            gantt.templates.rightside_text = function rightSideTextTemplate(start, end, task) {
                ...
                function ksGetTaskFitValue(task) {
                    ...
                    var textWidth = ((task.text || "").length + (gantt.config.ks_task_dynamic_progress ? (" (" + Math.round(task.progress * 100) + "%)").length : 0)) * gantt.config.font_width_ratio;
                    ...
                    if (state.drag_mode == modes.move || (state.drag_mode == modes.resize && !state.drag_from_start)) {
                        if (state.drag_mode == modes.move || (state.drag_mode == modes.resize && !state.drag_from_start)) {
                            if (ksGetTaskFitValue(task) === 'right') {
                                var ksTextWidth = ((task.text).length + 5) * gantt.config.font_width_ratio;
                                return "<div style='margin-left:"+ksTextWidth+"px'>"+dateToStr(gantt.roundDate(end))+"</div>";
                            }
...
//                        return dateToStr(gantt.roundDate(end));
            ...
            gantt.templates.task_text = function taskTextTemplate(start, end, task) {
                var ks_task_text = "";
                if (getTaskFitValue(task) === "center" || !gantt.ks_project_settings.ks_enable_task_dynamic_text) {
        ...
        ks_days_off_func: function () {
            gantt.ignore_time = function (date) {
                if (this.config.ks_hide_date && this.config.ks_days_off_selection && this.config.ks_days_off_selection.indexOf(date.getDay()) > -1)
                    return true;
                return false;
            };
        },
        ...
        ks_exclude_holiday_function: function () {
            ...
            if ( gantt.config.ks_days_off_selection) {
                for (var i = 0; i < gantt.config.ks_days_off_selection.length; i++) {
                    gantt.setWorkTime({ day: gantt.config.ks_days_off_selection[i], hours: false });
                }
            }
            ...
            for (var i = 0; i < gantt.config.ks_exclude_holiday.length; i++) {
                gantt.setWorkTime({
                    date: this.ksConversionUtcToLocal(moment(gantt.config.ks_exclude_holiday[i])),
                    hours: false
                });
            }
        ...
        destroy: function () {
            gantt.clearAll();
            _.each(gantt.config.ks_attached_events, function (ks_event) { gantt.detachEvent(ks_event); });
        ...
        ks_select_columns: function () {
            gantt.config.ks_attached_events.push(gantt.attachEvent("onScaleClick", function (e, date) {
        ...
        ks_gantt_overlay_canvas: function () {
            ...
            var lineOverlay = overlayControl.addOverlay(function (container) {
                ...
                if (values.planned.length > 1000) {
                    this.do_warn(_t("We are not able to show graph for large duration, please change the view scale"));
            ...
            function calculateResourceLoad(tasks, scale) {
                ...
                for (var i = 0; i < tasks.length; i++) {
                    ...
                    while (currDate < task.end_date) {
                        ...
                        if (step == 'hour' && (scale.step == 2 || scale.step == 4 || scale.step == 8)) {
                        ...
                        if (task.start_date > ks_scale_start_date && task.start_date < currDate)
                            ks_start_hours = new Date(task.start_date);
                        if (task.start_date < ks_scale_start_date && task.end_date > ks_scale_start_date)
                            ks_start_hours = new Date(ks_scale_start_date);
                        ...
                        while (ks_start_hours < ks_end_hours) {
                            if (Object.keys(task.ks_resource_hours_available).length) {
                                ...
                                if (ks_avail_hours && ks_avail_hours.indexOf(ks_start_hours.getHours()) > -1) {
                                    timegrid[timestamp] += 1;
                                }
                            }
                ...
                if (gantt.config.ks_resource_mode == 'tasks') {
                    for (var i in timegrid) {
                        ...
                        for (var ks_index = 0; ks_index < tasks.length; ks_index++) {
                            ...
                            else if(start.getTime() < tasks[ks_index].start_date.getTime() &&
                                tasks[ks_index].start_date.getTime() < end.getTime()){
                                    ks_count_task += 1;
                            }
                            else if (tasks[ks_index].start_date.getTime() < start.getTime() &&
                                end.getTime() <= tasks[ks_index].end_date.getTime()) {
                                    ks_count_task += 1;
                            }
                            else if (tasks[ks_index].start_date.getTime() < start.getTime() &&
                                tasks[ks_index].end_date.getTime() <= end.getTime() &&
                                tasks[ks_index].start_date.getTime() != end.getTime() && start < tasks[ks_index].end_date.getTime()){
                                    ks_count_task += 1;
                            }
                        }
            ...
            if (!gantt.config.rtl) {
                gantt.config.layout.rows.push(
                    { resizer: true, width: 1, next: "resources" },
                    {
                        height: 35,
                        cols: [
                            { html: "", group: "grids", width: gantt.config.ks_grid_width, min_width: gantt.config.ks_grid_width, },
                            { resizer: true, width: 1 },
                            { html: '<label> Hours ' + ks_resource_mode_hours.outerHTML + '</label> <label> Tasks ' + ks_resource_mode_tasks.outerHTML + '</label>', css: "resource-controls" }
                        ]
                    },
                    {
                        id: "resources",
                ...
                if (gantt.config.ks_resource_mode == 'tasks' || gantt.config.ks_resource_mode == 'hours' || gantt.config.ks_resource_mode === undefined) {
        ...
        ks_gantt_plugins: function(){
            ...
            gantt.plugins({
                ...
                quick_info: (this.ks_model_name == 'project.project' || this.ks_enable_quickinfo_extension) ? true : false,
                fullscreen: true,
                overlay: true,
            });
        },
        ...
        ks_left_grid_columns: function() {
            var ks_col_header = QWeb.render('ks_gantt_view_base.ks_col_header'),
                ks_col_content_buttons = function (task) {
                    return ( QWeb.render('ks_gantt_view_base.ks_col_content_buttons', {task: task, create: (this.ks_gantt_config && this.ks_gantt_config.ks_hide_sub_task_icon == "True") ? false : this.ks_isCreate}));
                }.bind(this);
            // Column defined here.
                ...
                },
                {
                    ...
                    template: function (obj) {
                        ...
                        if (obj.mark_as_important == "1") {
                            ks_column_str += "<div class='ks-gv-mark-as-important'><i class='fa fa-star'></i></div>"
                        }
...
//                        }
                        ...
                        ks_task_difference += (ks_rem_hours < 10 ? '0' + ks_rem_hours : ks_rem_hours) + ':';
        ...
        ks_set_scale_dropdown: function() {
            gantt.attachEvent("onGanttRender", function () {
                var ks_selected_zoom_level = gantt.ext.zoom.getLevels()[gantt.ext.zoom.getCurrentLevel()];
                var $ks_zoom_selection = $('select#ks_gantt_view_zoom');
                if ($ks_zoom_selection.length && ks_selected_zoom_level.name != $ks_zoom_selection[0].value) {
        ...
        ks_handle_selected_columns: function() {
            ...
            function ks_is_selected_column(column_date) {
                if (gantt.config.ks_selected_column && column_date.valueOf() == gantt.config.ks_selected_column.valueOf()) {
                    return true;
                }
                return false;
            }
        ...
        ks_handle_non_working_timeline: function() {
            ...
            gantt.templates.timeline_cell_class = function (item, date) {
                ...
                if (item.ks_owner_task && item.ks_resource_hours_available && !item.ks_resource_hours_available[date.getDay()] && gantt.config.ks_enable_days_off) {
                    return "weekend";
                }
                if (item.ks_owner_task && item.ks_resource_hours_available && item.ks_resource_hours_available[date.getDay()] && gantt.config.ks_enable_days_off) {
                    if (item.ks_resource_hours_available[date.getDay()].indexOf(date.getHours()) < 0 && gantt.getScale().unit == 'hour') {
                        return "weekend";
                    }
                }
        ...
        ks_handle_gantt_view_marker: function() {
            ...
            if (this.ks_project_start) {
                ...
                gantt.addMarker({
                    start_date: this.ksConversionUtcToLocal(moment(this.ks_project_start)),
                    css: "status_line",
                    text: _("Start"),
                    title: gantt.ks_gantt_view_datetime_format(this.ksConversionUtcToLocal(moment(this.ks_project_start)))
                });
            }
            if (this.ks_project_end) {
                ...
                gantt.addMarker({
                    ...
                    css: "status_line",
                    text: _("End"),
                    title: gantt.ks_gantt_view_datetime_format(this.ksConversionUtcToLocal(moment(this.ks_project_end)))
                });
            }
        },
        ...
        ks_handle_task_side_content: function() {
            ...
            gantt.config.ks_task_extra_info = function draw_deadline(task) {
                ...
                if (constraintType != types.ASAP && constraintType != types.ALAP && task.constraint_date && task.constraint_date_enable) {
                    var dates = gantt.getConstraintLimitations(task);
                    if (dates.earliestStart) {
                        ks_earliestStart_exist = true;
                        ks_left_text_position += 40;
                        els.appendChild(ksRenderDiv(task, dates.earliestStart, 'constraint-marker earliest-start'));
                    }

                    if (dates.latestEnd) {
                        ks_right_text_position += 40;
                        els.appendChild(ksRenderDiv(task, dates.latestEnd, 'constraint-marker latest-end'));
                    }
                ...
                if (task.progress >= 1) {
                    ...
                    el.style.left = sizes.left - 50 - (ks_earliestStart_exist ? 30 : 0) + 'px';
                    el.style.top = sizes.top + 'px';
                    els.appendChild(el);
                }
                ...
                if (gantt.config.ks_no_of_days) {
                    ...
                    var sizes = gantt.getTaskPosition(task, task.start_date);
                    task_duration_el.style.left = sizes.left - ks_left_text_position + 'px';
                    task_duration_el.style.top = sizes.top + 'px';
//                    els.appendChild(task_duration_el);
                    ks_left_text_position += 30;
                }

                if (ksGetTaskFitValue(task) === "left" && gantt.ks_project_settings.ks_enable_task_dynamic_text) {
                    var el = document.createElement('div');
                ...
                if (ksGetTaskFitValue(task) === "right") {
                    ...
                    if (gantt.ks_project_settings.ks_enable_task_dynamic_text) {
                        ...
                        el.className = 'ks_task_left_text';
                        var sizes = gantt.getTaskPosition(task, task.start_date);
                        el.style.left = sizes.left + sizes.width + ks_right_text_position + (task.deadline && task.deadline.valueOf() && task.end_date.valueOf() > task.deadline.valueOf() ? 110 : 0) + (task.type == 'milestone' ? 20 : 0) + 'px';
                        el.style.top = sizes.top + 'px';
                        els.appendChild(el);
                    }
                }
        ...
        ks_control_panel_slider: function() {
            ...
            var ksGetMenuSize_new = function () {
                return $('.ks-menu').outerWidth();
            };
            var ksMenuSizeNew = ksGetMenuSize_new();

            $(window).on('resize', function () {
              ksMenuSizeNew = ksGetMenuSize_new();
            });
            ...
            if (this.ks_model_name == 'project.task' ||  this.ks_model_name == 'project.project'){
                if (ksMenuSizeNew < ksMenuWrapperSize){
...
//                    $(ksRightPaddle).removeClass('hidden');
            ...
            $(ksRightPaddle).on('click', function () {
                ...
                var ksMenuWrapperSize = ksGetMenuWrapperSize();
                var ksMenuInvisibleSize = ksMenuSize - ksMenuWrapperSize;
                $('.ks-menu').animate({ scrollLeft: ksMenuInvisibleSize }, ksScrollDuration);
            });
        ...
        ks_gantt_view_datetime_format: function(){
            ...
            gantt.ks_gantt_view_datetime_format = function(ks_input_datetime) {
                return moment(ks_input_datetime).format(gantt.config.ks_lang_datetime_format);
            }
        },
        ...
        ks_task_drag_and_drop: function() {
            ...
            if (this.state.groupedBy.length == 0 && this.ks_model_name != 'project.project') {
                gantt.config.order_branch = "marker";
                gantt.config.order_branch = true;
                if (this.ks_allow_subtasks)
            ...
            gantt.config.ks_attached_events.push(gantt.attachEvent("onRowDragStart", function(id, target, e) {
