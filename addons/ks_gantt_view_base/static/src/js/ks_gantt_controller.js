    ...
    var ksGanttController = AbstractController.extend({
        ...
        ksGanttConfig: function () {
            ...
            gantt.attachEvent("onBeforeLinkAdd", function (id, link) {
                ...
                if ((this.modelName != 'project.project') && (!parseInt(link.source) || !parseInt(link.target))) {
                    gantt.message({
                        type: "error",
                ...
                if (gantt.getTask(link.source).type == 'project' || gantt.getTask(link.target).type == 'project' || gantt.getTask(link.target).ks_group_lvl) {
                    gantt.message({
                        type: "error",
                ...
                if ((this.modelName == 'project.project') && (gantt.getTask(link.source).project_id != gantt.getTask(link.target).project_id)) {
                    gantt.message({
                        type: "error",
                        text: _t("You can't create link with other project")
                    });
                    return false;
                }
                return true;
            ...
            gantt.attachEvent("onBeforeRowDragEnd", function (id, parent, target) {
                ...
                for (var ks_task = 0; ks_task < ks_all_task.length; ks_task++) {
                    ks_task_update_task[ks_all_task[ks_task].id] = { 'id': ks_all_task[ks_task].id };
                    if (ks_all_task[ks_task].ks_allow_subtask)
                        ks_task_update_task[ks_all_task[ks_task].id].parent_id = ks_all_task[ks_task].parent
                    ...
                    if (ks_init_sequence >= ks_all_task[ks_task].sequence) {
                        ...
                        ks_task_update_task[ks_all_task[ks_task].id]['sequence'] = ks_init_sequence;
                    }
                    else {
                ...
                if (this.model.modelName == gantt.ks_model_name) {
                    this.model.ksUpdateParentSequence(ks_task_update_task).then(function (res) {
        ...
        ksGanttCreateDp: function (event) {
            var self = this;
            this.ks_dp = gantt.createDataProcessor(function (entity, action, data, id) {
                switch (action) {
                    case "update":
                        switch (entity) {
                            case "task":
                    ...
                    case "create":
                        switch (entity) {
                            case "link":
                                ...
                                if (_.where(gantt.config.ks_link_data, {
                                    'source': data.source,
                                    'target': data.target
                                }).length) {
                                    break;
        ...
        renderButtons: function ($node) {
            ...
            this.$buttons = $(QWeb.render('ks_gantt_view_base.control_panel_buttons', {'values': values}));
            if ($node) {
        ...
        _ksOpenDialog: function (KsresID, context) {
            ...
            else if (context && context.open_new_task_form && (this.modelName == 'project.task' || this.modelName == 'project.project')) {
        ...
        _KsDeleteTask: function (id) {
            ...
            return this._rpc(ksUnlinkData).then(function () {
                return this.reload();
            }.bind(this));
        },
        ...
        _ksExportReport: function (ev) {
            ...
            var ks_file_name = "gantt-chart-" + new Date().toLocaleString().replace(new RegExp('/', 'g'), '-').replace(new RegExp(', ', 'g'), "-").replace(new RegExp(':', 'g'), "-");
            ...
            if (ks_report_type == "excel" && this.ks_export_field) {
                ...
                ks_file_name += '.xml';
                gantt.exportToPrimaveraP6({ name: ks_file_name, skip_circular_links: false });
        ...
        _ksTaskAdd: function (ev) {
            ...
            if (task_id) {
                ...
                if (ks_gantt.type == 'project') {
                    ...
                    if (ks_gantt.ks_allow_subtask) context['default_parent_id'] = parseInt(task_id);
                }
            }
            this._ksOpenDialog(undefined, context);
        },
