...
class KsIrHttp(models.AbstractModel):
    ...
    def session_info(self):
        ...
        if request.env.user.has_group('base.group_user'):
            result['ks_breadcrumb_style'] = request.env.ref('ks_curved_backend_theme.ks_global_config_single_rec').ks_breadcrumb_style
            ...
            if ks_all_setting_scope.get('scope_ks_loaders', '').lower() == 'global':
                result['ks_current_loader'] = self.sudo().env.ref('ks_curved_backend_theme.ks_global_config_single_rec')['ks_loaders']
