odoo.define('ks_curved_backend_theme.KsActionManagerInherit', function(require) {
    "use strict";
    ...
    ActionManager.include({
        ...
        _onHistoryBack: function() {
            ...
            if (this.currentDialogController) {
                ...
                if (session['ks_split_view'] && session['ks_split_view'] != 'no_split' && this.ks_controller_stack && this.ks_controller_stack.length && controller_length == 1) {
        ...
        _pushController: function(controller) {
            ...
            if (!session['ks_split_view'] || session['ks_split_view'] == 'no_split' || controller.viewType != 'form' || !this.getCurrentController() || self.getCurrentController()['viewType'] != 'list' || (this.actions[controller['actionID']].res_model != this.actions[this.getCurrentController()['actionID']].res_model)) {
                if (controller.viewType == 'list' && !controller.widget.$el.find('.o_list_view').hasClass('Ks_list_view'))
                    controller.widget.$el.find('.o_list_view').addClass('Ks_list_view')
                // detach the current controller
                if (self.ks_controller_stack && self.ks_controller_stack.length) {
                    self.KsPrevControllers();
                ...
                $('.ks_splitted_page').insertBefore('<div id="separator"></div>');
                if (session['ks_split_view'] && session['ks_split_view'] == 'horizontal')
                    $('body').addClass('ks_splitter horizontal-split');
                else if (session['ks_split_view'] && session['ks_split_view'] == 'vertical')
                    $('body').addClass('ks_splitter');

                self.ks_controller_stack.push(controller.jsID);
                // append the new controller to the DOM
        ...
        _appendController: function(controller, custom_ele_=false) {
            ...
            if (custom_ele_ && custom_ele_.length) {
                if ($('.ks_splitted_page').length)
                    $('.ks_splitted_page').remove();
                ...
                if (session['ks_split_view'] && session['ks_split_view'] == 'vertical') {
                    if (session['ks_splitted_vertical_width'])
                        this.getCurrentController().widget.$el.find('.Ks_list_view').outerWidth(session['ks_splitted_vertical_width'])
                    ...
                    $('<div id="separator" class="custom_seperator"></div>').insertAfter(this.getCurrentController().widget.$el.find('.Ks_list_view'));
                } else {
                    if (session['ks_splitted_horizontal_height'])
                        this.getCurrentController().widget.$el.find('.Ks_list_view').outerHeight(session['ks_splitted_horizontal_height'])
                    Object.assign(options, {
                        axis: 'y',
                        start: function(event, ui) {
                    ...
                    $('<div id="separator" class="resize-hr"></div>').insertAfter(this.getCurrentController().widget.$el.find('.Ks_list_view'));
                }
                ...
                $('#separator').on("dragstop", function(event, ui) {
                    if (session['ks_split_view'] == 'vertical') {
                        ...
                        var val = {
                            'ks_split_vertical_list_width': isNaN(parseFloat($('.Ks_list_view').outerWidth(true))) ? false : parseFloat($('.Ks_list_view').outerWidth(true)),
                        }
                    } else {
                        $('.resize-hr').css({
                            'opacity': '1'
                        ...
                        var val = {
                            'ks_split_horizontal_list_height': isNaN(parseFloat($('.Ks_list_view').outerHeight(true))) ? false : parseFloat($('.Ks_list_view').outerHeight(true)),
                        }
                    }
                    ajax.jsonRpc("/ks_curved_backend_theme/save/ks_splitted_form_width", "call", {
                        'data': val
                    });
                });
        ...
        KsPrevControllers: function() {
            let self = this;
            _.map(_.uniq(self.ks_controller_stack), function(re) {
