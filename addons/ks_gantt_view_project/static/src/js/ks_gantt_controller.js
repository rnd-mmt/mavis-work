odoo.define("ks_gantt_view_project.Controller", function (require) {
    ...
    ksGanttController.include({
        _ksExportReport: function(ev){
            ...
            if (ks_report_type == 'excel' && (this.modelName == 'project.task' || this.modelName == 'project.project')) {
                framework.blockUI();
                this.getSession().get_file({
                    url: '/web/ksgantt/export/xlsx',
                    data: {
                        project_id: this.model.context.default_project_id ? this.model.context.default_project_id : false,
                    },
