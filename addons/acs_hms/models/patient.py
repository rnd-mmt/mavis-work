...
class ACSPatient(models.Model):
    ...
    # ACTION SMART BUTTON HEALTH SERVICE
    def action_open_healthservice(self):
        return {
    ...
    def _smart_rec_count(self):
        for rec in self:
            healthservice_count = self.env['acs.health_service'].search_count([('patient_id', '=', rec.id)])
            rec.healthservice_count = healthservice_count
    ...
    def _patient_evaluation_graph_data(self):
        for rec in self:
            ...
            rec.patient_hr_line_graph = json.dumps(rec.patient_evaluation_line_graph_datas('hr', records, 'Hear Rate'))
            rec.patient_rr_line_graph = json.dumps(rec.patient_evaluation_line_graph_datas('rr', records, 'RR'))
            rec.patient_systolic_bp_line_graph = json.dumps(rec.patient_evaluation_line_graph_datas('systolic_bp', records, 'Systolic BP Gauche'))
            rec.patient_diastolic_bp_line_graph = json.dumps(rec.patient_evaluation_line_graph_datas('diastolic_bp', records, 'Diastolic BP Gauche'))
            rec.patient_systolic_bp_droite_line_graph = json.dumps(
                rec.patient_evaluation_line_graph_datas('systolic_bp_droite', records, 'Systolic BP Droite'))
            rec.patient_diastolic_bp_droite_line_graph = json.dumps(
                rec.patient_evaluation_line_graph_datas('diastolic_bp_droite', records, 'Diastolic BP Droite'))
    ...
    systolic_bp = fields.Float(related="last_evaluation_id.systolic_bp", string="Systolic BP Gauche")
    diastolic_bp = fields.Float(related="last_evaluation_id.diastolic_bp", string="Diastolic BP Gauche")
    systolic_bp_droite = fields.Float(related="last_evaluation_id.systolic_bp_droite", string="Systolic BP Droite")
    diastolic_bp_droite = fields.Float(related="last_evaluation_id.diastolic_bp_droite", string="Diastolic BP Droite")
    ...
    patient_systolic_bp_droite_line_graph = fields.Text(compute='_patient_evaluation_graph_data')
    patient_diastolic_bp_droite_line_graph = fields.Text(compute='_patient_evaluation_graph_data')
    ...
    #COUNT HEALTH SERVICE
    healthservice_count = fields.Integer(compute='_smart_rec_count', string='Service santé')
    imaging_ids = fields.One2many('patient.imaging.test', 'patient_id', string="Antécédents examen d' imagerie")
